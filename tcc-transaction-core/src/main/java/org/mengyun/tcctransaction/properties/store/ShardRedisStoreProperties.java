...
public class ShardRedisStoreProperties extends AbstractJedisStoreProperties {
    ...
    public ShardedJedisPool getShardedJedisPool() {
        if (instance == null) {
            List<JedisShardInfo> shardInfos = nodes.stream().map(node -> {
                JedisShardInfo info = new JedisShardInfo(node.getHost(), node.getPort(), getConnectionTimeout(), getSoTimeout(), 1);
