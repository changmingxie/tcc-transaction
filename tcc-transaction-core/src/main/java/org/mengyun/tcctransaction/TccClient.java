...
public class TccClient implements TccService {
    ...
    public TccClient(ClientConfig clientConfig) {
        ...
        if (this.clientConfig.getStorageType() == StorageType.REMOTING) {
            ...
            remotingClient = new NettyRemotingClient(this.remotingCommandSerializer, this.clientConfig, new ServerAddressLoader() {
    ...
    private void initializeRemotingClient() {
        ...
        this.requestProcessExecutor = new ThreadPoolExecutor(this.clientConfig.getRequestProcessThreadSize(), clientConfig.getRequestProcessThreadSize(), 1000L * 60, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(this.clientConfig.getRequestProcessThreadQueueCapacity()), new ThreadFactory() {
